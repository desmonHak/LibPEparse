...
int main() {
    ...
    *(uint64_t *)(b->mem + text_section_off + 2) = data_section_vaddr;
    ...
    dynsym[1].st_name = 11; // offset en dynstr: "printf"
    ...
    // Añade secciones
    size_t idx_interp = elf_builder_add_section(b, ".interp", SHT_PROGBITS, SHF_ALLOC, b->mem + interp_off, sizeof(interp), base_vaddr + interp_off, 1, NULL, NULL);
    size_t idx_dynamic = elf_builder_add_section(b, ".dynamic", SHT_DYNAMIC, SHF_ALLOC | SHF_WRITE, b->mem + dynamic_off, sizeof(dynamic), base_vaddr + dynamic_off, 8, NULL, NULL);
    size_t idx_dynstr = elf_builder_add_section(b, ".dynstr", SHT_STRTAB, SHF_ALLOC, b->mem + dynstr_off, dynstr_size, base_vaddr + dynstr_off, 1, NULL, NULL);


    size_t idx_dynsym = elf_builder_add_section(
        b, ".dynsym", SHT_DYNSYM, SHF_ALLOC, b->mem + dynsym_off, sizeof(dynsym), base_vaddr + dynsym_off, 8, NULL, NULL
    );

    size_t idx_relaplt = elf_builder_add_section(b, ".rela.plt", SHT_RELA, SHF_ALLOC, b->mem + rela_plt_off, sizeof(rela), base_vaddr + rela_plt_off, 8, NULL, NULL);
    ...
    uint64_t plt_addr = 0; // No hay PLT real, pero el linker usará la reloc
    *(uint32_t *)(b->mem + text_section_off + 14) = 0;

    elf_builder_finalize_exec64(
        b, text_section_vaddr, text_section_off, text_section_vaddr, code_size
    );
