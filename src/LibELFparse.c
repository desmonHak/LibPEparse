...
char **elf_dynsym_export_names(const ElfFile *elf, int *count) {
    ...
    if (elf->elf_class == ELFCLASS_32) {
        Elf32_Header *eh32 = elf->ehdr32;
        Elf32_Shdr *shdr = (Elf32_Shdr *)((uint8_t *)elf->mem + eh32->e_shoff);
        const size_t nsec = eh32->e_shnum;
        int ds = -1;
        ...
        if (ds < 0) return NULL;
        Elf32_Shdr *sym_sh = &shdr[ds];
        if (sym_sh->sh_entsize == 0 || sym_sh->sh_link >= nsec) return NULL;
        Elf32_Shdr *str_sh = &shdr[sym_sh->sh_link];
        if ((size_t)sym_sh->sh_offset + sym_sh->sh_size > elf->size ||
            (size_t)str_sh->sh_offset >= elf->size)
            return NULL;
        const size_t n = (size_t)(sym_sh->sh_size / sym_sh->sh_entsize);
        Elf32_Sym *syms = (Elf32_Sym *)((uint8_t *)elf->mem + sym_sh->sh_offset);
    ...
    if (elf->elf_class != ELFCLASS_64) return NULL;
